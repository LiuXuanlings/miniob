...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  vector<Row>          &rows          = insert_oper.rows();
  InsertPhysicalOperator *insert_phy_oper = new InsertPhysicalOperator(table, std::move(rows));
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  FieldMeta *fields = update_oper.fields();
  Value     *values      = update_oper.values();
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(table, fields, values));
  if (child_physical_oper) {
    oper->add_child(std::move(child_physical_oper));
  }
  return rc;
